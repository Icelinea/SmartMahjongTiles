...
class MahjongGame:
    ...
    def _setup(self):
        """初始配牌"""
        for i in range(4):
            self._players_hand[i] = [self.wall.live_wall.pop(0) for _ in range(13)]
            self._players_hand[i].sort()

    def _hand_sort(self, idx):
        suit_priority = {"m": 0, "p": 1, "s": 2, "z": 3}
        self._players_hand[idx].sort(key=lambda tile: (suit_priority[tile[1]], int(tile[0])))
    ...
    def _run_turn(self):
        ...
        self._players_hand[p_idx].append(new_tile)
        if self.debug_mode:
            logger.debug(f"玩家 {p_idx} 摸牌: {new_tile}")

        self._hand_sort(p_idx)
        if self.debug_mode:
            logger.debug(f"当前手牌: {self._players_hand[p_idx]}")

        # 2. 玩家决策
        discard_tile = self._players_hand[p_idx].pop() 
    ...
    def run(self, _turn=10):
        for _ in range(_turn):
            time.sleep(10)
            if not self._run_turn(): break